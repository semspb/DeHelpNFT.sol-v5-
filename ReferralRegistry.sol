...
contract ReferralRegistry is AccessControl {
    ...
    function bindSponsor(
        ...
        // Optional: owner check (если нужно)
        require(
            nft.ownerOf(tokenId) == nft.ownerOf(tokenId),
            "Owner mismatch"
        );

        // Cycle protection (lightweight)
    ...
    function getUpline(
        uint256 tokenId,
        uint256 depth
    ) external view returns (uint256 sponsorTokenId) {
    ...
    function getFullUpline(
        uint256 tokenId
    ) external view returns (uint256[] memory upline) {
